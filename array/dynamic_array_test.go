...
func TestDynamicArray(t *testing.T) {
	...
	t.Run("Add Element into Dynamic Array", func(t *testing.T) {
		...
		if !reflect.DeepEqual(numbers.GetAll(), res) {
			t.Errorf("Expected be [10, 20, 30, 40, 50] but got %v", numbers.GetAll())
		}
	})
	...
	t.Run("Remove in Dynamic Array", func(t *testing.T) {
		...
		if !reflect.DeepEqual(numbers.GetAll(), res) {
			t.Errorf("Expected be [10, 30, 40, 50] but got %v", numbers.GetAll())
		}
	})
